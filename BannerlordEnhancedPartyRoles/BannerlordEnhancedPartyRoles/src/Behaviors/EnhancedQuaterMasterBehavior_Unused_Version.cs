...
namespace BannerlordEnhancedPartyRoles.Behaviors
{
    ...
    class QuaterMasterDialog : CampaignBehaviorBase
    {
        ...
        {
            starter.AddPlayerLine("QauerMaster_Ontmoeting", "hero_main_options", "quatermaster_continue_conversation", "Give Companions best items from my inventory", EnhancedQuaterMasterService_Unused_Version.IsQuaterMaster, null);
		...
		public static void GiveBestEquipmentFromItemRoster()
		{
			...
			List<Hero> allCompanionsHeros = EnhancedQuaterMasterService_Unused_Version.GetCompanionsHeros(mainParty.Party.MemberRoster.GetTroopRoster(), mainParty.LeaderHero);

			var tuple = EnhancedQuaterMasterService_Unused_Version.UpdateCompanionsArmour(itemRoster.ToList(), allCompanionsHeros);
